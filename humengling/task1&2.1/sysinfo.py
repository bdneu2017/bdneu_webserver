...
class DataBase():
    ...
    def db_con(self):  # 建立与数据库的连接，若不存在表data或alert则创建表，返回cur
        ...
        for table_name in self.t_list:
            table_namel = "('" + table_name + "',)"
            # make the format same to compare
            if table_namel in str(tables):  # 存在表
                pass
            elif table_name == 'data':
                cur.execute('create table data(r_time varchar(20),\
                    cpu_rate varchar(5), mem_rate varchar(5),\
                    dis_rate varchar(20), nes_rate varchar(20),\
                    ner_rate varchar(20))')
            elif table_name == 'alert':
                cur.execute('create table alert(r_time varchar(20),\
                    alert_name varchar(20),alert_data varchar(20))')
        return [con, cur]
...
class Alert(object):
    ...
    def def_log(self):
        ...
        formatter = logging.Formatter('%(asctime)s - %(name)s - %(levelname)s -\
            %(message)s')  # define print formatter
    ...
    def alert(self, logger, result, cur):  # 打印警告信息并将信息存入数据库
        ...
        if cpu > cpu_ts:
            ...
            cur.execute('insert into alert values (%s, %s, %s)',
                        [tim, "cup_use", cpu])
        if mem > mem_ts:
        ...
        if nes > net_ts:
            ...
            cur.execute('insert into alert values(%s, %s, %s)',
                        [tim, "net_send_rate", nes])
        if ner > net_ts:
            logger.warning(tim + "  net_recv_rate  " + str(ner))
            cur.execute('insert into alert values(%s, %s, %s)',
                        [tim, "net_recv_rate", ner])
# thread
...
if __name__ == '__main__':
    ...
    dis_ts = 65
    net_ts = 5000.0
    print '|  time(y-m-d h:m:s)  | cpu utilization rate (%)|\
            memory utilization rate (%)| disk utilization rate (%)|\
            net sent rate & net recv rate (B/s)|'
    db = DataBase('root', '123456', 'test', '127.0.0.1', ['data', 'alert'])
    [con, cur] = db.db_con()
    q = Queue.Queue()
    info = Info(q)
    alert = Alert()
    logger = alert.def_log()
    f_list = [info.cpu_urate, info.memory_urate, info.disk_urate,
              info.netp_rate, info.now_time]
    try:
        while True:
            thread = GetResultByThread(f_list, q)
            result = thread.thread()
            print '%-22s %-25s %-29s %-26s %-15s %-20s'\
                  % (result[0], result[1], result[2], result[3],
                      result[4], result[5])  # real-time printing in format
            # and write all data to database and alert
