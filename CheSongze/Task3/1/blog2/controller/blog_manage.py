...
from keywords import keynum,wordreplace
...
def keys():
    ...
    for i in range(len(key)):
        key[i].times+=1
    key = Keywords.query.get(0)
    key.times = 1
    db.session.add(key)
    db.session.commit()
    #db.session.commit()
    return render_template('key.html',entries=key)
'''
@app.route('/entries/')
#@login_required
def show_entries():
...
def add_entry():
    ...
    category.content = request.form['text']
    category.userid=current_user.id
    category.username=current_user.login
